...
async def up(db_url, to=LATEST, dir=os.path.dirname(__file__)):
    ...
    for migration, record in migrations_to_run:
        if not migration:  # Check if migration is None
            print(f"Migration is None, skipping...")
            continue

        if record and migration != record:
            message = f"Incompatible migration history at {migration.name if migration else 'Unknown'}"
